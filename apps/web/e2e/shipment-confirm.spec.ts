...
// Extends the shipment quote journey through confirming the shipment --
// same caveat as registration.spec.ts: creates real throwaway staging data
// with no automated cleanup yet.
test("seller can confirm a shipment and see its drop-off deadline", async ({ page }) => {
  const uniqueEmail = `e2e-confirm-${Date.now()}@faira-test.dev`;
  ...
  await page.getByLabel("Full name").fill("E2E Confirm Seller");
  ...
  // Seller pays -> lands in AWAITING_PAYMENT, not AWAITING_DROPOFF.
  await page.getByRole("radio", { name: /^I'll pay/ }).click();
