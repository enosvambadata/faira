...
// Stripe POSTs application/json, so the global express.json() has already
// captured the exact bytes on rawBody — which is what constructEvent must
// verify against (never the parsed req.body). Handles Vamba Shipping customer
// card payments (checkout.session.completed -> mark PAID, idempotently).
