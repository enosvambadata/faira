import { request, ApiError, API_URL, createClient } from "@faira/ui";

// Faira Fulfilment web API client. The HTTP core (request/ApiError/auth) and
// the Supabase browser client live in @faira/ui; this file holds the
// fulfilment-specific types + methods (onboarding, verification, hubs, hub-ops,
// shipments, transport, manifests, tracking).
export { auth } from "@faira/ui";
export { ApiError };
export { ApiError as FulfilmentApiError };

// A few methods talk to Supabase directly rather than through request().
