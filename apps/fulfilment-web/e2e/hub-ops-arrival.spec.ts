...
// E2E_ADMIN_TOKEN is set. Everything up to "dispatched and in transit"
...
test.describe("hub-ops destination-hub arrival scanning", () => {
  test.skip(!ADMIN_TOKEN, "E2E_ADMIN_TOKEN not set -- this spec provisions real staff via the admin API");

  test("destination hub agent can scan an arriving parcel in, generating a collection code", async ({ page, request }) => {
    // Full seller->seal->dispatch cycle over real network round trips to
    // staging + Cloudinary before the browser is ever touched.
    test.setTimeout(150_000);
    const uniqueId = Date.now();

    const bootstrapEmail = `e2e-arrival-bootstrap-${uniqueId}@faira-test.dev`;
    ...
    const sellerEmail = `e2e-arrival-seller-${uniqueId}@faira-test.dev`;
    ...
    const originStaffEmail = `e2e-arrival-origin-staff-${uniqueId}@faira-test.dev`;
    ...
    await request.post(`${API_URL}/api/v1/fulfilment/hub-ops/shipments/${shipment.id}/seal`, {
      headers: { Authorization: `Bearer ${originStaffToken}` },
      data: { sealNumber: `SEAL-ARRIVAL-${uniqueId}` },
    });

    // --- Ops admin schedules a run, origin staff creates+finalizes the manifest, scans it out ---
    const adminEmail = `e2e-arrival-opsadmin-${uniqueId}@faira-test.dev`;
    ...
    const runRes = await request.post(`${API_URL}/api/v1/fulfilment/transport/runs`, {
      ...
      data: {
        ...
        vehicleReference: `ARRIVAL-${uniqueId}`,
      },
    });
    ...
    // --- Destination-hub staff drives the actual arrival-scan UI ---
    const destStaffEmail = `e2e-arrival-dest-staff-${uniqueId}@faira-test.dev`;
    ...
    await page.goto("/hub-ops/arrival");
    await page.getByLabel("Shipment reference").fill(confirmed.reference);
    await page.getByRole("button", { name: "Scan in" }).click();

    await expect(page.getByText("is ready for collection.")).toBeVisible({ timeout: 15_000 });
    await expect(page.getByText(confirmed.reference).first()).toBeVisible();
  });
});
